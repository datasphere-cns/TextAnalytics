...
custom.stopwords<- read.csv('data/lista de stopwords.csv')
cuantificadores <- read.csv('data/cuantificadores.csv')
sustantivos <- read.csv('data/sustantivos.csv')
...
write.csv(dataframe.corpus.limpio, file = "data/", row.names = F)
corpus
...
corpus2 <- read.csv('data/CleanCorpus.csv')
