classify_polarity <- function(textColumns,algorithm="bayes",pstrong=0.5,pweak=1.0,prior=1.0,verbose=TRUE,...) {
  ...
  lexicon <- read.csv("data/subjectivityStemming2.csv",header=FALSE)
	...
	for (i in 1:nrow(matrix)) {
		...
		if (round(scores$positive,4)>round(scores$negative,4)) 
		{
		  best_fit <- "positive"
		  compos <- compos + 1
		}
		else if (round(scores$positive,4)<round(scores$negative,4))
		{
		  best_fit <- "negative" 
		  comneg <- comneg + 1
		}
		
		else if (all(round(scores$positive,4) == round(scores$negative,4)) == TRUE)
		{
		  best_fit <- "neutral" 
		  comneu <- comneu + 1
		}
