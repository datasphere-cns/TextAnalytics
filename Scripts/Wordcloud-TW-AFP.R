...
## AFP Crecer
DATASET_AFPCrecer<- read_csv( "C:/Users/nelso/Downloads/AWS/AWS/NLP/Reports/TW-ENE-JUL-Neg.csv", 
...
comentsbind <- comentsbind[ !(comentsbind$autor %in% c('@AFPCrecer','@AFPCONFIA' )), ]
...
wordcloud(words = dfgrouped_AFPcrecer$token, freq = dfgrouped_AFPcrecer$m, min.freq = 5, 
...
graph <- bigramasCrecer %>%
  ...
  filter(n > 4) %>% graph_from_data_frame(directed = FALSE)
set.seed(123)
