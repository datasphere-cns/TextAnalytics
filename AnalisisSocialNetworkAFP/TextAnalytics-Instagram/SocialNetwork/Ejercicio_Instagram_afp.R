...
coment_tidy <- coment_tidy[ !(coment_tidy$token %in% c('afpconfia','afpcrecer','','$$','+','=','<U+FFFD>')), ]

ggplot(coment_tidy, aes(x = as.Date(fecha), fill = empresa)) +
  geom_histogram(position = "identity", bins = 20, show.legend = FALSE) +
  scale_x_date(date_labels = "%m-%Y", date_breaks = "1 month") +
  labs(x = "fecha de publicaci?n", y = "n?mero de comentario") +
...
lista_stopwords_esp <- read_csv("D:/Github/Datasphere/TextAnalytics/AnalisisSocialNetworkAFP/TextAnalytics-Instagram/SocialNetwork/UTF8_spanish.csv", col_names = TRUE) 
...
# Selecci?n de variables
df_AFPCrecerbigrama <- instagram_AFPCrecer %>% select(Name, Date, Comment, account)
